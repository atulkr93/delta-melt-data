import os
pb_dir = [0.01, 0.05, 0.1, 0.15, 0.20, 0.25, 0.30, 0.40, 0.45, 0.50, 0.75, 1.0, 2.0, 3.0, 5.0, 8.0]
...
+
Name scaf2-CGT_GC8_high_5.4.csv 
lf 150.7
AlignMag AUTO
resn 1.0 
eqbn N
...
for dummy in range(len(pb_dir)):
    ...
    os.system('cp ../scaf2-CGT_GC8_high_5.4.csv .')
    os.system('cp ../scaf2-CGT_GC1p_high_5.4.csv .')
    os.system('cp ../cest_fit_final_v10.py .')
    f = open('BMNS_params.txt', 'w')
    f.write(opfile%(pb_decimals[dummy], pb_decimals[dummy]))
    f.close()
    os.system('nohup python cest_fit_final_v10.py BMNS_params.txt 2state > /dev/null &')
    os.chdir('..')
