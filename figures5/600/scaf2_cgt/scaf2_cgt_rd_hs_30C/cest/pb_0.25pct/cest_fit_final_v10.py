...
def simulate_cest_offset(expt_var, offset_value, *fit_args):
    ...
    BM_mat = MatrixBM_3state(k12, k21, k13, k31, k23, k32, delta_a[0], delta_b[0], delta_c[0], slps[0], r1, r2, r1b, r2b, r1c, r2c, pa, pb, pc)
    ...
    for dummy in range(slps.shape[0]):
        ...
        BM_mat = MatrixBM_3state(k12, k21, k13, k31, k23, k32, delta_a[dummy], delta_b[dummy], delta_c[dummy], slps[dummy], r1, r2, r1b, r2b, r1c, r2c, pa, pb, pc)
...
def plot_profiles(fit_answer, rchi2):
    ...
    for dummy_slp in number_slps:
        ...
        plt.errorbar((offset_vals/(2*math.pi*params['lf'])), params['norm_intensity'][slp_positions], yerr=params['norm_intensity_error'][slp_positions], color = colors_plot[counter], label='%4.2f'%float(dummy_slp/(2*math.pi)), linewidth=0, marker='o', markersize=6)
        # Plot fit vals
        ...
        # plot trend lines
        plt.errorbar((offset_vals_trendline/(2*math.pi*params['lf'])), intensity_trendline, color = colors_plot[counter], linewidth=3)
    ...
    fig.savefig(str(output_foldername) + "/fit-" + str(params['Name'][:params['Name'].index('.csv')]) + ".pdf")
    plt.close(fig)
...
def mc_error_sets(intensity_answer, initial_vals, lbounds, ubounds):
    ...
    def fit_iter(iter_number):    
        ...
        return popt
    popt_matrix = parmap(fit_iter, range(mc_iter), 4) 
