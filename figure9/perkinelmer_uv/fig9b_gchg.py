...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        sequence_context = dummy_outer + 'g' + dummy_inner

        #if sequence_context not in ["AGG", "TGG"]:
        # Get wc delG
        wc_file = 'scaf2_' + sequence_context + '_gc_ph5.4hsnb/' + 'test.csv'
        ...
        # Get m1GC delG
        m1g_file = 'scaf2_' + sequence_context + '_m1gc_ph5.4hsnb/' + 'test.csv'
...
deldelG_deltamelt = deldelG + ufloat(-0.11785224142595088, 0.07012200117127347) 

#print "deldelG stats", np.amin(deldelG), np.amax(deldelG)
#print len(delG_m1g_values)
f = open('m1g_25c_new.csv', 'w')
...
# Plotting
# reference relative to CGT
deldelG_deltamelt = deldelG_deltamelt - deldelG_deltamelt[7]
fig, ax = plt.subplots()
plot_subplot(ax, sequence_contexts, deldelG_deltamelt, '$\Delta \Delta G^{\circ}_{conf,25 ^{\circ} C}$' + ' (kcal/mol)', [0.0, 3.0, 1.0], '%d', 'Sequence Contexts')
title = ax.set_title('G-C' + '$^{+}$' + ' Hoogsteen', fontsize=fs)
title.set_position([0.5, 1.02])


plt.tight_layout()
#plt.show()
plt.savefig('fig9b_gchg.pdf')
