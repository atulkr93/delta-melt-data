...
nontwo_state = ['aaa', 'aag', 'cac', 'gac', 'gag', 'tag']
sequence_contexts = []
delG_wc_values = []
delG_m1g_values = []
...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        # Get m1GC delG
        m1g_file = 'scaf2_' + sequence_context + '_m3t_done/final_all/' + 'test.csv'
...
deldelG_deltamelt = deldelG + ufloat(2.3925574916234074, 0.135317797408615)
#print "deldelG stats", np.amin(deldelG), np.amax(deldelG)
#print len(delG_m1g_values)
f = open('m3t_25c_new.csv', 'w')
...
# reference relative to CAG
deldelG_deltamelt = deldelG_deltamelt - deldelG_deltamelt[6]

# Plotting
fig, ax = plt.subplots()
plot_subplot(ax, sequence_contexts, deldelG_deltamelt, '$\Delta\Delta G^{\circ}_{conf,25 ^{\circ} C}$' + ' (kcal/mol)', [0.0, 2.0, 1.0], '%d', 'Sequence Contexts')
title = ax.set_title("A-T Opening", fontsize=fs)
title.set_position([0.5, 1.02])

# intra-nn correlations   
plt.tight_layout()
#plt.show()
plt.savefig('fig9b_atopen.pdf')
