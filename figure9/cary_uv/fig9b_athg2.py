...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        # Get wc delG
        wc_file = 'scaf2_' + sequence_context + '_at_done/final_all/' + 'test.csv'
        ...
        # Get m1GC delG
        m1g_file = 'scaf2_' + sequence_context + '_m1a_done/final_all/' + 'test.csv'
...
f = open('m1a_25c_new2.csv', 'w')
f.write('context,deldelG_25C,deldelG_25C_error,delG_m1g_25C,delG_m1g_25C_error,delG_wc_25C,delG_wc_25C_error,deldeH,deldelH_error,deldelS,deldelS_error\n')
for dummy in range(len(deldelG)):
    print sequence_contexts[dummy], deldelG_deltamelt[dummy] 
    f.write(sequence_contexts[dummy] + "," + str(unumpy.nominal_values(deldelG_deltamelt[dummy])) + "," + str(unumpy.std_devs(deldelG_deltamelt[dummy])) + "," + str(-1.0*unumpy.nominal_values(delG_m1g_values[dummy])) + "," + str(unumpy.std_devs(delG_m1g_values[dummy])) + "," + str(-1.0*unumpy.nominal_values(delG_wc_values[dummy])) + "," + str(unumpy.std_devs(delG_wc_values[dummy])) + "," + str(unumpy.nominal_values(deldelH[dummy])) + "," + str(unumpy.std_devs(deldelH[dummy])) + "," + str(unumpy.nominal_values(deldelS[dummy])) + "," + str(unumpy.std_devs(deldelS[dummy])) + "\n")
f.close()
...
title = ax.set_title('m1A-T', fontsize=fs)
