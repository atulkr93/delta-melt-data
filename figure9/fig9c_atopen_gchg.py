...
m1a = pd.read_csv('perkinelmer_uv/m1g_25c_new_at.csv')
m3t = pd.read_csv('cary_uv/m3t_25c_new.csv')

context_exclude_m1a = ['aag', 'tag']
...
#plt.title(eqn)
plt.text(5.3, 6.6, 'r = ' + '%3.2f'%np.corrcoef(m1a['deldelG_25C'].values, m3t['deldelG_25C'].values)[0,1], fontsize=fs)
#plt.plot([2.0, 5.5], [2.0 + popt[0], 5.5 + popt[0]], color='k', linewidth=2)
...
plt.xlabel('$\Delta G_{conf,25 ^{\\circ}}$' + ' (kcal/mol)' + '\nG-C' + '$^{+}$' + ' Hoogsteen', fontsize=fs)
#plt.ylabel('$\Delta G_{conf,25 ^{\\circ}}$' + ' (kcal/mol)' + ' A-T Opening', fontsize=fs)
plt.ylabel('A-T Opening', fontsize=fs)
plt.xlim([3, 6])
plt.ylim([6.5, 9.])
plt.xticks([3.0, 4.0, 5.0, 6.0], fontsize=fs)
plt.yticks([7.0, 8.0, 9.0], fontsize=fs)
plt.savefig('fig9c_gchg_atopen_corr.pdf')
#plt.show()
