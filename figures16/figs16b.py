...
def fit_data(filename, temperature, absorbance, mode, Ct):
    ...
    if mode == 'hairpin': 
        ...
        print "Duplex"
        p0 = np.array([0.001, 0.5, 0.001, 0.7, -70, 310])
        ...
        plt.tick_params(axis='x', direction='out', which='both', length=6, width=2, top=False)
        plt.tick_params(axis='y', direction='out', which='both', length=6, width=2, right=False)
        #fig.suptitle(str(filename), fontsize=fl)
        #ax[0].set_ylabel("Population", fontsize=fl)
        #ax[0].set_xlabel("Temperature (" + "$^\circ$" + "C)", fontsize=fl)
        ...
        # absorbance
        #ax.set_title('$\chi^{2}$' + ' ' + str('%.3f'%redchi2))
        ax.set_ylabel('Absorbance', fontsize=fl)
        ax.set_yticks([0.60, 0.65, 0.70, 0.75])
        ax.set_ylim([0.60, 0.78])
        ax.set_yticklabels(['%.2f'%ele for ele in [0.60, 0.65, 0.70, 0.75]], fontsize=fl)
        ax.set_xlabel("Temperature (" + "$^\circ$" + "C)", fontsize=fl)
        ax.plot(np.linspace(5,100,500), uv_duplex(np.linspace(5,100,500), *popt), color='b', label='2-state fit', linewidth=4)
        ax.plot(temperature, absorbance, color='k', linewidth=0, label='Data', marker='o', markersize=4)
        ax.set_xticks(np.arange(0.0, 100.1, 20))
        ax.set_xticklabels(["%d"%ele for ele in np.arange(0.0, 100.1, 20)], fontsize=fl)
        ax.set_xlim([t_dummy[0]-273.16, t_dummy[-1]-273.16])
        ax.legend(fontsize=fl, loc=2)

        plt.savefig('figs16b.pdf')
...
def caller():
    ...
    labels_net = []
    filenames_net = []
    for filename in ['../figure9/cary_uv/scaf2_aag_m1a_done/final_all/20210128_probe11_melting8_aag_m1a.txt']:
        if filename[-3:] == "txt":
            print "Reading ", filename
            [labels, result] = read_data(filename, 'duplex', 3.0)
            labels_net = labels
            if result_net == []:
    ...
    for dummy in range(np.shape(result_net)[0]):
        data_line = str(filenames_net[dummy]) + "," + str(result_net[dummy][0]) + ''.join(["," + str(result_net[dummy][dummy_int]) for dummy_int in range(1, np.shape(result_net)[1])]) + ',' + 'duplex' + '\n'
	f.write(data_line)
  
    # Now, write the stats
    avg_line = "avg," + str(result_avg[0]) + ''.join(["," + str(result_avg[dummy_int]) for dummy_int in range(1, np.shape(result_net)[1])]) + ',' + 'duplex' + '\n'
    f.write(avg_line)
