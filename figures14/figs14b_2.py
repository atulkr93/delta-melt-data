...
bf = pd.read_csv('../figure9/perkinelmer_uv/m1g_25c_new.csv')
contexts_exclude = ['agg', 'tgg']
...
plot_contexts(bf, ax, 'g', contexts_exclude, np.arange(3.0, 6.1, 1.0), '%d', 'G-C' + '$^{+}$' + ' Hoogsteen\n' + '$\Delta\Delta G_{delta-melt, 25^{\\circ}C}^{\\circ}$'+ ' (kcal/mol)')

plt.savefig('figs14b_2.pdf')
#plt.show()
