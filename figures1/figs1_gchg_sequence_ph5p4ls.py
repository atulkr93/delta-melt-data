...
def plot_uv(ax, dirname, filename, yaxislabel, ylim, yinterval, xlim, xinterval, xaxisticks_status, fit_type, mode):
    ...
    for dummy in range(len(bf['filename'])-2):
        ...
        ax[counter].set_xticks(np.arange(15, xlim[dummy][1]+1, 15))
        if xaxisticks_status == 1:
            ax[counter].set_xticklabels(['%d'%ele for ele in np.arange(15, xlim[dummy][1]+1, 15)], fontsize=fs)
        else:
...
fig, axarr = plt.subplots(2, 6, figsize=(44.1, 14.6))
fig.suptitle('pH 5.4', fontsize=fs)
plot_uv(list(axarr[0, :3]), '../figure9/perkinelmer_uv/scaf2_cgt_gc_ph5.4lsnb/', 'test.csv', 'scaf2_CGT' + '$^{GC}$', [[0.60, 0.74], [0.60, 0.74],[0.60, 0.73]], 0.05, [[10, 100.0], [10., 100.], [10., 100.]], 20.0, 1, '2state', 'duplex')
plot_uv(list(axarr[0, 3:]), '../figure9/perkinelmer_uv/scaf2_cgt_m1gc_ph5.4lsnb/', 'test.csv', 'scaf2_CGT' + '$^{m1GC}$', [[0.59, 0.73], [0.59, 0.73], [0.59, 0.73]], 0.05, [[10, 100.0], [10., 100.], [10., 100.]], 20.0, 1, '2state', 'duplex')
plot_uv(list(axarr[1, :3]), '../figure9/perkinelmer_uv/scaf2_tgc_gc_ph5.4lsnb/', 'test.csv', 'scaf2_TGC' + '$^{GC}$', [[0.56, 0.69], [0.56, 0.69],[0.56, 0.69]], 0.05, [[10, 100.0], [10., 100.], [10., 100.]], 20.0, 1, '2state', 'duplex')
plot_uv(list(axarr[1, 3:]), '../figure9/perkinelmer_uv/scaf2_tgc_m1gc_ph5.4lsnb/', 'test.csv', 'scaf2_TGC' + '$^{m1GC}$', [[0.58, 0.69], [0.64, 0.76],[0.58, 0.71]], 0.05, [[10, 100.0], [10., 100.], [10., 100.]], 20.0, 1, '2state', 'duplex')
...
#plt.show()
plt.tight_layout()
plt.savefig('figs1_gchg_sequence_ph5p4ls.pdf')
