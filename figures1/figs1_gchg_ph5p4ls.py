...
def plot_uv(ax, dirname, filename, yaxislabel, ylim, yinterval, xlim, xinterval, xaxisticks_status, fit_type, mode):
    ...
    colors_plot = ['k', 'b', 'r']
    t_dummy = np.linspace(0.0, 100.0, 500)   
    counter = 0
    ...
    for dummy in range(len(bf['filename'])-2):
        ...
        if master <= 2:
            ax[counter].set_xticks(np.arange(20, 70, 20))
        else:
            ax[counter].set_xticks(np.arange(xlim[dummy][0]+xinterval, xlim[dummy][1]+1, xinterval))
        if xaxisticks_status == 1:
            if master <= 2:
                ax[counter].set_xticklabels(['%d'%ele for ele in np.arange(20, 70, 20)], fontsize=fs)
            else:
...
fig, axarr = plt.subplots(5, 6, figsize=(44.1, 35))
fig.suptitle('pH 5.4', fontsize=fs)
master = 0
plot_uv(axarr[0, :3], '../figure5_figure10/a2dna_ph5.4nb/', 'test.csv', 'A' + '$_{2}$' + '-DNA', [[0.5, 0.64], [0.5, 0.64], [0.5, 0.64]], 0.05, [[10, 70.0], [10., 70.], [10., 70.]], 15.0, 1, '2state', 'duplex')
plot_uv(axarr[0, 3:], '../figure5_figure10/a2dna_ph5.4_hongluebuffer_10pctd2o/', 'test.csv', 'A' + '$_{2}$' + '-DNA*', [[0.61, 0.75], [0.57, 0.73], [0.67, 0.82]], 0.05, [[10.0, 70.0], [10.0, 70.0], [10.0, 70.0]], 15.0, 1, '2state', 'duplex')
plot_uv(axarr[1, :5], '../figure5_figure10/a2dna_m1g10_ph5.4nb/', 'test.csv', 'A' + '$_{2}$' + '-DNA' + '$^{m1G10}$', [[0.56, 0.72], [0.56, 0.72], [0.56, 0.72], [0.56, 0.72], [0.56, 0.72]], 0.05, [[10, 70.0], [10., 70.], [10., 70.], [10., 70.], [10., 70.]], 15.0, 1, '2state', 'duplex')
plot_uv([axarr[1, 5]] + list(axarr[2, :3]), '../figure5_figure10/a6dna_ph5.4/', 'test.csv', 'A' + '$_{6}$' + '-DNA', [[0.52, 0.71], [0.52, 0.72], [0.52, 0.72], [0.52, 0.71]], 0.05, [[0, 90.0], [0., 90.], [0., 90.], [0., 90.]], 20.0, 1, '2state', 'duplex')
plot_uv(axarr[2, 3:], '../figure5_figure10/a6dna_m1g10_keck_ph5.4nb/', 'test.csv', 'A' + '$_{6}$' + '-DNA' + '$^{m1G10}$', [[0.53, 0.70], [0.53, 0.70], [0.53, 0.70]], 0.05, [[0., 90.0], [0., 90.], [0., 90.]], 20.0, 1, '2state', 'duplex')
plot_uv(list(axarr[3, :4]), '../figure5_figure10/a6dna_rg10_ph5.4/', 'test.csv', 'A' + '$_{6}$' + '-DNA' + '$^{rG10}$', [[0.52, 0.72], [0.52, 0.72], [0.52, 0.72], [0.52, 0.72]], 0.05, [[0, 90.0], [0., 90.], [0., 90.], [0., 90.]], 20.0, 1, '2state', 'duplex')
plot_uv(list(axarr[3, 4:]) + list(axarr[4, :2]), '../figure5_figure10/a6dna_m1rg10_ph5.4/', 'test.csv', 'A' + '$_{6}$' + '-DNA' + '$^{m1rG10}$', [[0.50, 0.69], [0.50, 0.71], [0.50, 0.70], [0.50, 0.70]], 0.05, [[0, 90.0], [0., 90.], [0., 90.], [0., 90.]], 20.0, 1, '2state', 'duplex')
...
#plt.show()
plt.tight_layout()
plt.savefig('figs1_gchg_5p4.pdf')
