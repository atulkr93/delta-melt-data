...
def plot_uv(ax, dirname, filename, yaxislabel, ylim, yinterval, xlim, xinterval, xaxisticks_status, fit_type, mode):
    ...
    for dummy in range(len(bf['filename'])-2):
        ...
        ax[counter].set_yticks(np.arange(ylim[dummy][0]+yinterval, ylim[dummy][1], yinterval))
        ax[counter].set_yticklabels(['%3.2f'%ele for ele in np.arange(ylim[dummy][0]+yinterval, ylim[dummy][1], yinterval)], fontsize=fs)
        #if counter == 0:
        #else:
...
fig, axarr = plt.subplots(3, 6, figsize=(44, 20))
fig.suptitle('pH 6.8', fontsize=fs)
plot_uv(axarr[0, :3], '../figure3/a2dna_ph6.8hb/', 'test.csv', 'A' + "$_{2}$" + "-DNA", [[0.57, 0.74], [-0.01, 0.14], [0.56, 0.72]], 0.05, [[10, 90.0], [10., 90.], [10., 90.]], 15.0, 1, '2state', 'duplex')
plot_uv(axarr[0, 3:], '../figure3/a2dna_m1a16_pH6.8hb/', 'test.csv', 'A' + "$_{2}$" + "-DNA" + "$^{m1A16}$", [[0.54, 0.71], [0.57, 0.74], [0.59, 0.78]], 0.05, [[10, 90.0], [10., 90.], [10., 90.]], 15.0, 1, '2state', 'duplex')
plot_uv(axarr[1, :3], '../figure3/a6dna_ph6.8/', 'test.csv', 'A' + "$_{6}$" + "-DNA", [[0.54, 0.72], [0.54, 0.72], [0.52, 0.70]], 0.05, [[10, 90.0], [10., 90.], [10., 90.]], 15.0, 1, '2state', 'duplex')
plot_uv(axarr[1, 3:], '../figure3/a6dna_m1a16_ph6.8/', 'test.csv', 'A' + "$_{6}$" + "-DNA" + "$^{m1A16}$", [[0.54, 0.72], [0.53, 0.73], [0.54, 0.72]], 0.05, [[10, 90.0], [10., 90.], [10., 90.]], 15.0, 1, '2state', 'duplex')
plot_uv(axarr[2, :3], '../figure3/a6dna_m1a21_ph6.9hb/', 'test.csv', 'A' + "$_{6}$" + "-DNA" + "$^{m1A21}$", [[0.55, 0.75], [0.55, 0.75], [0.55, 0.75]], 0.05, [[10, 90.0], [10., 90.], [10., 90.]], 15.0, 1, '2state', 'duplex')

axarr[0, 0].set_ylabel('A' + '$_{260}$', fontsize=fs)
axarr[1, 0].set_ylabel('A' + '$_{260}$', fontsize=fs)
axarr[2, 0].set_ylabel('A' + '$_{260}$', fontsize=fs)
...
#    axarr[1, dummy].set_xlabel('Temperature (' + '$^{\circ}$' + 'C)', fontsize=fs)
    axarr[2, dummy].remove()

#plt.show()
plt.tight_layout()
plt.savefig('figs1_athg_6p8_2.pdf')
