...
def plot_curve(ax, dirname, data, R1w, E, xlim, xticks, xfmt, ylim, yticks, yfmt, plot_title, add_status, base_conc):
    ...
    ax.plot(kex_array, residuals_net, linewidth=elw, color='k')
    ax.plot([unumpy.nominal_values(kex), unumpy.nominal_values(kex)], [0., math.pow(10, 4)], linewidth=elw, linestyle="--", color='k')
    ax.set_yscale('log')
    ax.set_xscale('log')
    ax.set_ylim([0.0, math.pow(10, 4)])
    ax.set_yticks([math.pow(10, ele) for ele in range(5)])
    ax.set_yticklabels(['$10^{%d}$'%int(ele) for ele in range(5)], fontsize=fs)
    ax.set_xticks([math.pow(10, ele) for ele in range(int(math.ceil(math.log10(np.amin(kex_array)))), int(math.floor(math.log10(np.amax(kex_array))))+1)])
    ax.set_xticklabels(['$10^{%d}$'%int(ele) for ele in range(int(math.ceil(math.log10(np.amin(kex_array)))), int(math.floor(math.log10(np.amax(kex_array))))+1)], fontsize=fs)
    ax.set_xlim([np.amin(kex_array), np.amax(kex_array)])
...
def plot_fit(ax, x, y):
    ...
    ax.errorbar(x, yvals, yerr=yerrors, color='k')
    ax.plot([x[0], x[-1]], [(x[0]*m_avg) + b_avg, (x[1]*m_avg) + b_avg], color='r')
...
kex_array = list(np.logspace(-1., 2, 10.)) + [2.65] 
plot_curve(ax[1, 1], 'A6DNA_8.8_25C_20mM_Resi_6/', 'data_A6DNA_8.8_25C_20mM_Resi_6.csv', 0.334388287, 1.853923, [-0.2, 3.2], [0, 3., 1], '%d', [0., 1.05], [0., 1.01, 0.2], '%2.1f', 'A' + '$_{6}$' + '-DNA T6-H3 20mM NH' + '$_{3}$', 1, 20)
kex_array = list(np.logspace(-1., 2, 10.)) + [3.94] 
plot_curve(ax[1, 2], 'A6DNA_8.8_25C_40mM_Resi_6/', 'data_A6DNA_8.8_25C_40mM_Resi_6.csv', 0.329595522, 1.782285, [-0.2, 3.2], [0, 3., 1], '%d', [-0.1, 1.05], [0., 1.01, 0.2], '%2.1f', 'A' + '$_{6}$' + '-DNA T6-H3 40mM NH' + '$_{3}$', 1, 40)
kex_array = list(np.logspace(-1., 3, 10.)) + [5.75] 
plot_curve(ax[1, 3], 'A6DNA_8.8_25C_100mM_Resi_6/', 'data_A6DNA_8.8_25C_100mM_Resi_6.csv', 0.331505686, 1.951047, [-0.1, 1.1], [0, 1.0, 0.5], '%2.1f', [-0.3, 1.1], [0., 1.01, 0.5], '%2.1f', 'A' + '$_{6}$' + '-DNA T6-H3 100mM NH' + '$_{3}$', 1, 100)
kex_array = list(np.logspace(-1., 3, 10.)) + [6.66] 
plot_curve(ax[1, 4], 'A6DNA_8.8_25C_150mM_Resi_6/', 'data_A6DNA_8.8_25C_150mM_Resi_6.csv', 0.320788387, 1.929775, [-0.1, 1.1], [0, 1.0, 0.5], '%2.1f', [-0.3, 1.1], [0., 1.01, 0.5], '%2.1f', 'A' + '$_{6}$' + '-DNA T6-H3 150mM NH' + '$_{3}$', 1, 150)
...
for dummy in range(5):
    ...
    ax[3, dummy].set_xlabel('$k$' + "$_{ex}$" + ' (s' + '$^{-1}$' + ')', fontsize=fs)
#for dummy in range(5):
