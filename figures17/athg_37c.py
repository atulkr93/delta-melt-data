...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        delG_wc = delH_wc - ((273.16+37)*delS_wc)
        ...
        delG_m1g = delH_m1g - ((273.16+37)*delS_m1g)
        ...
        # Contexts with non-two state behavior, set all params to 0
        # so that in plot, they are negative, and don't appear
...
def plot_subplot(ax, xlabels, yvals, ylabel, ylims, fmt, xaxislabel):
    ...
    ax.set_xticklabels(xlabels, rotation='vertical', fontsize=fs)
...
deldelG_deltamelt = deldelG_deltamelt - deldelG_deltamelt[10]

# Plotting
fig, ax = plt.subplots()
plot_subplot(ax, sequence_contexts, deldelG_deltamelt, '$\Delta \Delta G^{\circ}_{conf,25 ^{\circ} C}$' + ' (kcal/mol)', [0.0, 4.5, 1.0], '%d', 'Sequence Contexts')
title = ax.set_title('A-T Hoogsteen', fontsize=fs)
title.set_position([0.5, 1.02])


plt.tight_layout()
#plt.show()
#plt.savefig('fig9b_athg.pdf')
