...
# Plotting
fig, ax = plt.subplots()
plot_subplot(ax, sequence_contexts, delG_wc_values, '$\Delta G^{\circ}_{melt,25 ^{\circ} C}$' + ' (kcal/mol)', [0.0, 18.0, 4.0], '%d', 'Sequence Contexts')
title = ax.set_title('A-T' + " base pairs", fontsize=fs)
title.set_position([0.5, 1.02])


plt.tight_layout()
#plt.show()
plt.savefig('figs12b.pdf')
