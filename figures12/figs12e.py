...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        # Get wc delG
        wc_file = '../figure9/perkinelmer_uv/scaf2_' + sequence_context + '_gc_ph5.4hsnb/' + 'test.csv'
        ...
        # Get m1GC delG
        m1g_file = '../figure9/perkinelmer_uv/scaf2_' + sequence_context + '_m1gc_ph5.4hsnb/' + 'test.csv'
...
delG_wc_values = delG_wc_values * -1.0

# Plotting
fig, ax = plt.subplots()
plot_subplot(ax, sequence_contexts, delG_wc_values, '$\Delta G^{\circ}_{melt,25 ^{\circ} C}$' + ' (kcal/mol)', [0.0, 19.0, 4.0], '%d', 'Sequence Contexts')
title = ax.set_title('G-C' + " base pairs", fontsize=fs)
title.set_position([0.5, 1.02])


plt.tight_layout()
#plt.show()
plt.savefig('figs12e.pdf')
