...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        # Get m1GC delG
        m1g_file = '../figure9/cary_uv/scaf2_' + sequence_context + '_m1a_done/final_all/' + 'test.csv'
        ...
        if sequence_context in nontwo_state:
            delG_wc_values.append(0.0)
            delG_wc_error_values.append(0.0)
            delG_m1g_values.append(0.0)
...
#    print sequence_contexts[dummy], deldelG_deltamelt[dummy] 
...
title = ax.set_title('m1A-T base pairs', fontsize=fs)
title.set_position([0.5, 1.02])


#nn = pd.read_csv('gc_nn_flip.csv')
#nn_wc_delH = []
#nn_wc_delG = []
#nn_wc_delS = []
#for dummy in range(len(sequence_contexts)):
#    nn_subset = nn.loc[nn['DNA_wt_Comp(5p3p)'] == 'A' + sequence_contexts[dummy][3:6] + 'G']
#    nn_wc_delH.append(nn_subset['DNA_WT_delH(kcal/mol)'].iloc[0])
#    nn_wc_delS.append(nn_subset['DNA_wt_delS(kcal/mol/K)'].iloc[0])
#    nn_wc_delG.append(nn_subset['DNA_WT_delG(kcal/mol)'].iloc[0])
#    print sequence_contexts[dummy][3:6], nn_subset['DNA_wt_Comp(5p3p)'].iloc[0], nn_wc_delH[-1], nn_wc_delS[-1], nn_wc_delG[-1]

# intra-nn correlations   
#plot_corr(ax[0, 0], nn_wc_delH, nn_wc_delS, [65, 90, 10], [0.15, 0.25, 0.05], "$-\Delta H_{nn,GC}(kcal/mol)$", "$-\Delta S_{nn,GC}(kcal/mol/K)$", "%d", "%.2f") 
#plot_corr(ax[0, 1], nn_wc_delG, nn_wc_delH, [65, 90, 10], [0.15, 0.25, 0.05], "$-\Delta G_{nn,GC}(kcal/mol)$", "$-\Delta H_{nn,GC}(kcal/mol)$", "%d", "%.2f") 
#plot_corr(ax[1, 0], nn_wc_delG, nn_wc_delS, [65, 90, 10], [0.15, 0.25, 0.05], "$-\Delta G_{nn,GC}(kcal/mol)$", "$-\Delta S_{nn,GC}(kcal/mol/K)$", "%d", "%.2f") 

# nn - UV correlations
#plot_corr(ax[0], delH_wc_values, nn_wc_delH, [65, 90], [70, 90, 10], [31, 37], [31, 37, 2], "$-\Delta H_{GC}(kcal/mol)$", "$-\Delta H_{nn,GC}(kcal/mol)$", "%d", "%d") 
#plot_corr(ax[1], delS_wc_values, nn_wc_delS, [0.17, 0.25], [0.17, 0.25, 0.03], [0.086, 0.093], [0.086, 0.093, 0.003], "$-\Delta S_{GC}(kcal/mol)$", "$-\Delta S_{nn,GC}(kcal/mol)$", "%.2f", "%.3f") 

#plot_corr(ax[0], -1*delG_wc_values, deldelG, [15, 18], [15, 18, 1], [5.5, 9.0], [6.0, 9.0, 1.0], "$-\Delta G_{GC}(kcal/mol)$", "$\Delta\Delta G_{CHARTT}(kcal/mol/K)$", "%d", "%d") 

plt.tight_layout()
#plt.show()
plt.savefig('figs12b_2.pdf')
