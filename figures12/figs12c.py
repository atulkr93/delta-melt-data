...
for dummy_outer in ['a', 'c', 'g', 't']:
    for dummy_inner in ['a', 'c', 'g', 't']:
        ...
        if sequence_context in nontwo_state:
            ...
            delG_wc_error_values.append(0.)
            delG_m1g_values.append(-2.0)
            delG_m1g_error_values.append(0.)

            delH_wc_values.append(0.0)
            delH_wc_error_values.append(0.)
            delH_m1g_values.append(0.0)
            delH_m1g_error_values.append(0.)

            delS_wc_values.append(0.0)
            delS_wc_error_values.append(0.)
            delS_m1g_values.append(0.0)
            delS_m1g_error_values.append(0.)
...
# Plotting
fig, ax = plt.subplots()
plot_subplot(ax, sequence_contexts, deldelG_deltamelt, '$\Delta G^{\circ}_{conf,25 ^{\circ} C}$' + ' (kcal/mol)', [0.0, 6.5, 1.0], '%d', 'Sequence Contexts')
